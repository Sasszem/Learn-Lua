...
token = strsep(&str, ":");
token = strsep(&str, ":");
int line=atoi(token)-2;
g_print("Error in line %d\n",line+2);
tag_error_line(line);

    GtkWidget *dia
=gtk_message_dialog_new_with_markup (GTK_WINDOW(gtk_builder_get_object(builder,"window")),
                                    GTK_DIALOG_MODAL,
                                    GTK_MESSAGE_ERROR,
                                    GTK_BUTTONS_OK,
                                    "ERROR:\n%s",
...
int run_user_code(lua_State *L)
{
    ...
    char *text;
    gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(gtk_builder_get_object(builder,"program_buffer")), &start, &end);
    text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(gtk_builder_get_object(builder,"program_buffer")), &start, &end, 0);
    int error;
